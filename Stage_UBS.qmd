...
point_16 <- df_filter_2d[16, ]
point_500 <- df_filter_2d[500, ]

mu_1 <- as.numeric(filter_2d$mu_hat[16, ])  
sigma_1 <- filter_2d$V_hat[,,16]

mu_2 <- as.numeric(filter_2d$mu_hat[500, ])  
sigma_2 <- filter_2d$V_hat[,,500]

x_seq1 <- seq(df_filter_2d$mu_hat_x[16] - 10, df_filter_2d$mu_hat_x[16] + 10, by = .1)
y_seq1 <- seq(df_filter_2d$mu_hat_y[16] - 10, df_filter_2d$mu_hat_y[16] + 10, by = .1)

x_seq2 <- seq(df_filter_2d$mu_hat_x[500] - 10, df_filter_2d$mu_hat_x[500] + 10, by = .1)
y_seq2 <- seq(df_filter_2d$mu_hat_y[500] - 10, df_filter_2d$mu_hat_y[500] + 10, by = .1)


grille_points1 <- expand.grid(x = x_seq1, y = y_seq1)
grille_points2 <- expand.grid(x = x_seq2, y = y_seq2)

dist_maha1 <- mapply(get_mahalanobis, 
                     x = grille_points1$x, 
                     y = grille_points1$y, 
                     MoreArgs = list(mu = mu_1, sigma = sigma_1))

dist_maha2 <- mapply(get_mahalanobis, 
                     x = grille_points2$x, 
                     y = grille_points2$y, 
                     MoreArgs = list(mu = mu_2, sigma = sigma_2))

area_1 <- grille_points1[dist_maha1 <= q, ]

area_2 <- grille_points2[dist_maha2 <= q, ]

ggplot()+
  geom_point(data = df_filter_2d,
             aes(x = mu_hat_x, y = mu_hat_y),
             size = 0.4)+
geom_point(data = point_16,
             aes(x = mu_hat_x, y = mu_hat_y),
             size = 2, col = 'red') +
  geom_point(data = point_500,
             aes(x = mu_hat_x, y = mu_hat_y),
             size = 2, col = 'blue') +
  geom_tile(data = area_1, aes(x = x, y = y), fill = "red", alpha = .4)+
  geom_tile(data = area_2, aes(x = x, y = y), fill = "blue", alpha = .4)+
...
```{r}
x_min <- point_16$mu_hat_x - 5  
x_max <- point_16$mu_hat_x + 5  
y_min <- point_16$mu_hat_y - 5  
y_max <- point_16$mu_hat_y + 5  
...
```{r}
x_min <- point_500$mu_hat_x - 5  
x_max <- point_500$mu_hat_x + 5  
y_min <- point_500$mu_hat_y - 5  
y_max <- point_500$mu_hat_y + 5  
