...
get_SIR <- function(target_model, # A list with elements ..... 
  ...
  log_w0 <- log(target_model$get_x0_knowing_y0(particles[,,1], y_obs[1,], pars)) -
    log(q_proposal$get_initial_density(particles[,,1], pars)) # get the log initials weights for the n_particles x_0 ------- the log is needed to process the log likelihood
  ...
  for (i in 2:n_obs) { # start of the loop
    ...
    if(current_ESS <= threshold){ # compare threshold and ESS -------- threshold = 0 -> SIS | threshold = n_particles + 1 or NULL -> SIR | 0 < threshold < n_particles + 1 -> SIR_bis
      log_old_weights <- rep(0, n_particles) # initalisation of a n_particles vector
      ancestor_indexes <- sample(1:n_particles,
                                 size = n_particles,
                                 replace = TRUE,
                                 prob = filtering_weights[i - 1, ]) # sample n_particles from the index ancestor_indexes with proba filtering_weights[i-1,] for each number from the index
    }
    ...
    filtering_weights[i,] <- exp(unnormed_log_filtering_weights[i, ] - log_sum_unnormed_w) # get by the exp the real weight from the log weights
